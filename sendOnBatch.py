...
for idx0,fl in enumerate(fileChunks):
	...
	if opts.put != "":
		#print >> sh, '[ "${EXIT}" == "0" ] && { cmsMkdir ' + opts.put +'  && cmsStage -f ${WORKDIR}/output.root ' + opts.put + '/output_%(idx)d.root  && touch sub_%(idx)d.done || echo "cmsStage fail" > sub_%(idx)d.fail; }'%{'idx':idx}
		print >> sh, '[ "${EXIT}" == "0" ] && { ' + EOS + " mkdir " + "/eos/cms"+opts.put +'  ; cmsStage -f ${WORKDIR}/output.root ' + opts.put + '/output_%(idx)d.root  && touch sub_%(idx)d.done || echo "cmsStage fail" > sub_%(idx)d.fail; }'%{'idx':idx}
	else:
		print >> sh, '[ "${EXIT}" == "0" ] && { cp ${WORKDIR}/output.root ./output_%(idx)d.root && touch sub_%(idx)d.done || echo "cp fail" > sub_%(idx)d.fail ; }'%{'idx':idx}
